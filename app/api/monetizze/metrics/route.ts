...
export async function GET() {
  try {
    ...
    const abandonedEvents = events.filter((e: any) => {
      const m = typeof e.metadata === 'string' ? JSON.parse(e.metadata || '{}') : e.metadata || {}
      return e.eventType.includes('abandon') || m.status === 'abandoned' || m.status === 'refused' || m.status === 'chargeback'
    })
    ...
    console.error('Erro ao buscar métricas Monetizze:', error)
