...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = generateCacheKey(session.user.id, 'tiktok-metrics', { period })
    const cached = cache.get(cacheKey)
    if (cached) return NextResponse.json(cached)

    const integration = await prisma.integration.findFirst({
      where: { userId: session.user.id, platform: 'TIKTOK_ADS', isActive: true },
    })
    ...
    if (config?.demo === true) {
      ...
      const events = funnel
        ? await prisma.funnelEvent.findMany({
            where: {
              funnelId: funnel.id,
              eventType: { in: ['tiktok_click', 'tiktok_ad_click'] },
              timestamp: { gte: since },
            },
      ...
      if (ilegiveis > 0) {
        console.error(`[tiktok] ${ilegiveis} de ${events.length} eventos com metadata ilegível; cliques e gasto estão SUBESTIMADOS.`)
      }
      if (impressions === 0) impressions = clicks * 28
      ...
      const response = {
        ...
        raw: { impressions, clicks, spend, reach: Math.round(impressions * 0.65) },
    ...
    return NextResponse.json({
      connected: true,
      cpm: 'R$ 0.00', cpc: 'R$ 0.00', ctr: '0%', impressoes: '0', cliques: 0, gastos: 'R$ 0.00',
      error: 'Integração TikTok Ads conectada, mas sem fornecedor de dados em tempo real.',
    })
  } catch (error) {
    console.error('Erro ao buscar métricas TikTok:', error)
