...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  try {
    ...
    const rl = await checkRateLimit(`webhook:eduzz:${token}`, 120, 60_000)
    ...
    const integration = await findIntegrationByWebhookToken('EDUZZ', token)
    if (!integration) {
    ...
    console.log('📦 Eduzz webhook (tokenized):', JSON.stringify(sanitizeForLog(body)))
    await processEduzzEvent(body, integration.userId, startTime, `/api/webhooks/eduzz/${token.slice(0, 8)}…`)
    return NextResponse.json({ success: true })
  } catch (error) {
    console.error('Erro webhook Eduzz (tokenized):', error)
