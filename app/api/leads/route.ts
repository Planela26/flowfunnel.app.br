...
export async function GET(request: Request) {
  try {
    ...
    // ── Por que TrackedLead NÃO entra aqui ────────────────────────────────────
    //
    // Cheguei a juntar `TrackedLead` nesta lista e estava errado. Lead é quem
    // deixou um meio de contato — e-mail ou telefone. `TrackedLead` guarda
    // NAVEGADOR, não pessoa: um registro por visitante anônimo, incluindo
    // robô e rastreador de busca. Misturar os dois enche a aba de linhas sem
    // contato e infla a contagem a ponto de ela não significar mais nada:
    // centenas de "leads" para um punhado de cliques reais no anúncio.
    //
    // Visitante vira lead quando deixa contato — e aí ele chega por webhook,
    // que é o que esta rota já lê. O volume de visitantes tem lugar próprio:
    // o card da Landing Page e a aba Rastreamento, onde é rotulado como
    // visita, não como lead.
    //
    // A guarda acima (`if (!phone && !email) continue`) é a regra, e vale
    // para qualquer fonte que venha a ser somada aqui no futuro.
